...
interface CardProps {
  ...
  export function AboutOverviewCard({ card, index, cycleCard, totalCards }: CardProps) {
    const zIndex = totalCards - index
    const yOffset = index * -15 // Vertical offset
    const xOffset = index * 30 // Horizontal offset
  
    // Track drag direction
    const [dragDirection, setDragDirection] = useState<{ x: number; y: number }>({ x: 0, y: 0 })
    ...
    return (
      ...
      >
        ...
        >
          ...
          {index === 0 && <div className="absolute inset-0 z-10 cursor-grab active:cursor-grabbing" />}
        </div>
      </motion.div>
    )
  }
